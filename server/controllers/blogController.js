  ...
  try {
    ...
    if (knowledgeDocs && knowledgeDocs.length > 0) {
      knowledgeContext = knowledgeDocs
        .slice(0, 3)
        .map((doc) => `[Grounding Material: ${doc.fileName}]\n${doc.extractedText.slice(0, 1000)}...`)
        .join('\n\n');
    }
...
exports.updateBlog = async (req, res, next) => {
  try {
    ...
    if (blog.status === 'scheduled') {
      ...
      const targetPlatform = publishPlatform || (blog.publishInfo && blog.publishInfo.platform) || 'html';
      const existingSchedule = await Schedule.findOne({ blogId: blog._id, status: 'scheduled' });
      if (existingSchedule) {
        existingSchedule.scheduledDate = blog.publishDate;
        existingSchedule.publishPlatform = targetPlatform;
        await existingSchedule.save();
      } else {
        await Schedule.create({
          companyId: req.user.companyId,
          blogId: blog._id,
          publishPlatform: targetPlatform,
          publishOptions: {},
      ...
      if (oldStatus === 'scheduled' || blog.status !== 'scheduled') {
